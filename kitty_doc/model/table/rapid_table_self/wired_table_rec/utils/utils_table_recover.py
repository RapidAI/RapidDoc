...
def plot_html_table(
    ...
    for point in logi_points:
        max_row = max(max_row, point[1] + 1)
        max_col = max(max_col, point[3] + 1)
    ...
    for i, logic_point in enumerate(logi_points):
        ...
        text = " ".join(ocr_rec_text_list) if ocr_rec_text_list else ""
        for row in range(row_start, row_end + 1):
            for col in range(col_start, col_end + 1):
                grid[row][col] = text

    # 创建压缩版表格：每行把所有非空 cell 合并
    ...
    for row in range(max_row):
        ...
        while col < max_col:
            ...
            if not cell_text:
                ...
                # 计算跨列长度
                span = 1
                while col + span < max_col and grid[row][col + span] == cell_text:
                    span += 1
                if span > 1:
                    temp_row += f"<td colspan={span}>{cell_text}</td>"
                else:
                    temp_row += f"<td>{cell_text}</td>"
                col += span
        temp_row += "</tr>"
        table_html += temp_row
