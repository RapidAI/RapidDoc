...
DOT_LEADER_RUN_MIN_LENGTH = 8
DOT_LEADER_MIN_NON_PUNCT_CHARS = 80
        ...
        with pdfium_guard():
            ...
            if ascii_punct_signal["triggered"]:
                logger.debug(
                    ...
                    f"effective_ascii_punct_ratio="
                    f"{ascii_punct_signal['effective_ascii_punct_ratio']:.4f}, "
                    f"effective_punct_run_ratio="
                    f"{ascii_punct_signal['effective_punct_run_ratio']:.4f}"
                )
                return "ocr"
...
def _count_dot_leader_chars(text: str) -> int:
    """统计目录页常见的点引导线，避免把目录误识别为 ASCII 标点乱码。"""
    dot_leader_chars = 0
    current_run = 0
    ...
    for char in text:
        ...
        if current_run >= DOT_LEADER_RUN_MIN_LENGTH:
            dot_leader_chars += current_run
        current_run = 0

    if current_run >= DOT_LEADER_RUN_MIN_LENGTH:
        dot_leader_chars += current_run

    return dot_leader_chars
...
def _get_sampled_ascii_punct_signal_from_samples(text_samples):
    ...
    best_signal = {
        ...
        "punct_run_ratio": 0.0,
        "dot_leader_chars": 0,
        "effective_ascii_punct_ratio": 0.0,
        "effective_punct_run_ratio": 0.0,
    }
    ...
    for text_sample in text_samples:
        ...
        dot_leader_chars = _count_dot_leader_chars(cleaned_text)
        non_punct_chars = cleaned_text_chars - ascii_punct_count

        effective_ascii_punct_count = ascii_punct_count
        effective_ascii_punct_run_chars = ascii_punct_run_chars
        if non_punct_chars >= DOT_LEADER_MIN_NON_PUNCT_CHARS:
            effective_ascii_punct_count = max(0, ascii_punct_count - dot_leader_chars)
            effective_ascii_punct_run_chars = max(
                0,
                ascii_punct_run_chars - dot_leader_chars,
            )
        ...
        if cleaned_text_chars > 0:
            ...
            effective_ascii_punct_ratio = (
                effective_ascii_punct_count / cleaned_text_chars
            )
            effective_punct_run_ratio = (
                effective_ascii_punct_run_chars / cleaned_text_chars
            )
        ...
        signal = {
            ...
            "dot_leader_chars": dot_leader_chars,
            "effective_ascii_punct_ratio": effective_ascii_punct_ratio,
            "effective_punct_run_ratio": effective_punct_run_ratio,
        }
        if (
            cleaned_text_chars >= SUSPICIOUS_ASCII_PUNCT_MIN_TEXT_CHARS
            and effective_ascii_punct_ratio
            >= SUSPICIOUS_ASCII_PUNCT_RATIO_THRESHOLD
            and effective_punct_run_ratio
            >= SUSPICIOUS_ASCII_PUNCT_RUN_RATIO_THRESHOLD
        ):
            signal["triggered"] = True
            return signal
        ...
        if (
            signal["effective_punct_run_ratio"],
            signal["effective_ascii_punct_ratio"],
            signal["cleaned_text_chars"],
        ) > (
            best_signal["effective_punct_run_ratio"],
            best_signal["effective_ascii_punct_ratio"],
