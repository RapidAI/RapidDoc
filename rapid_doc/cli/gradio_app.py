...
from rapid_doc.cli.common import prepare_env, read_fn, aio_do_parse, pdf_suffixes, image_suffixes
...
async def to_markdown(file_path, end_pages=10, is_ocr=False, formula_enable=True, table_enable=True, language="ch", backend="pipeline", url=None):
    ...
    # 获取识别的md文件以及压缩包文件路径
    local_md_dir, file_name = await parse_pdf(file_path, '../../docker/output', end_pages - 1, is_ocr, formula_enable, table_enable, language, backend, url)
